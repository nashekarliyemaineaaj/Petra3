...
from Petra import dispatcher, telethn
from Petra.__main__ import (CHAT_SETTINGS, DATA_EXPORT, DATA_IMPORT,
                                   HELPABLE, IMPORTED, MIGRATEABLE, STATS,
                                   USER_INFO, USER_SETTINGS)
from Petra.modules.helper_funcs.chat_status import dev_plus, sudo_plus
...
def unload(update: Update, context: CallbackContext):
    ...
    try:
        imported_module = importlib.import_module("Petra.modules." +
                                                  text)
    except:
...
def listmodules(update: Update, context: CallbackContext):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("Petra.modules.", 1)[1]
